# KKU-python-tutorial

- [ตารางการอบรม](https://github.com/mrolarik/KKU-python-tutorial/blob/master/Python-Tutorial-KKU-Feb-2019.pdf)  


## โปรแกรมที่ใช้ในการเขียน Python
- [ดาวน์โหลดโปรแกรมภาษา Python](https://www.python.org)
- [การติดตั้งโปรแกรม Python](https://github.com/mrolarik/basic-python/wiki/Download-and-Installing-Python3)
- [ดาวน์โหลดโปรแกรม atom เพื่อใช้เขียนโปรแกรม](http://atom.io)


## Python Tutorial
- [ตัวแปร (variable)](https://github.com/mrolarik/KKU-python-tutorial/blob/master/variable.ipynb)
- [เงื่อนไขแบบ if-else (if-else condition)](https://github.com/mrolarik/basic-python/blob/master/if-else-condition.ipynb)
- [ตัวแปรประเภท list และการเข้าถึงข้อมูลใน list (list and index)](https://github.com/mrolarik/basic-python/blob/master/list_for_loop.ipynb)  
- [การเพิ่มข้อมูลเข้าไปใน list: append](https://github.com/mrolarik/basic-python/blob/master/list_append.ipynb)  
- [โปรแกรมคำนวณค่า BMI: ประยุกต์ใช้ list](https://github.com/mrolarik/basic-python/blob/master/bmi.ipynb)  
- [try-except](https://github.com/mrolarik/basic-python/blob/master/try_except.ipynb)
- [ฟังก์ชัน (function)](https://github.com/mrolarik/basic-python/blob/master/function-tutorial.ipynb)
- [การใช้ฟังก์ชันในการสร้างเมนู (Creating a menu by using function)](https://github.com/mrolarik/basic-python/blob/master/function_menu.ipynb)  

## Basic Python
- [basic python](https://github.com/mrolarik/basic-python)

![รูปอบรม](https://github.com/mrolarik/KKU-python-tutorial/blob/master/pic1.jpg)
![รูปอบรม](https://github.com/mrolarik/KKU-python-tutorial/blob/master/pic.jpg)
